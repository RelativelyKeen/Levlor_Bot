...
async def level(ctx):
    ...
    await ctx.send(embed=discord.Embed.from_dict({
      "title": f"**Rank:** #{rank}",
      "color": 0,
      "description": f"**Level:** {level}\n\n*Points left:* {pointsleft}/{pointsforlevel}",
      "timestamp": "",
      "author": {
        "name": f"{author}",
        "icon_url": f"{pfp}"
...
async def settings(ctx, setting, args):
    ...
    if setting == "points" and int(args) > 0:
        set_setting(ctx.guild.id, "points", int(args))
        await ctx.send(f'Amount of points in a level changed to: {str(args)}')
    ...
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send(content=None, embed=discord.Embed.from_dict({
             ...
             "fields": [
               ...
               {
                 "name": "Points",
                 "value": f"used to change the amount of points for a levle\nusage: {prefix}settings points <new amount>"
               }
             ]
           }))
